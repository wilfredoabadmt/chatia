 ...
 */
function candidatesBR(numDigits) {
    const out = new Set();
...
async function backfill(companyId, whatsappId, createMissing = false) {
    const rows = await Baileys_1.default.findAll({
        where: whatsappId ? { whatsappId } : undefined
    });
    let updated = 0;
    let created = 0;
    for (const row of rows) {
        const contacts = typeof row.contacts === "string"
        ...
        for (const c of contacts) {
            ...
            let contact = await Contact_1.default.findOne({
                where: { companyId, number: { [sequelize_1.Op.in]: cand } }
            });
