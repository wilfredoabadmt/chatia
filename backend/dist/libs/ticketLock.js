...
exports.acquireTicketWelcomeLock = void 0;
...
function createRedis() {
    const uri = process.env.IO_REDIS_URI ||
    ...
    if (uri) {
        return new ioredis_1.default(uri, common);
    }
    const host = process.env.REDIS_HOST || "127.0.0.1";
    const port = Number(process.env.IO_REDIS_PORT ||
        process.env.REDIS_PORT ||
        5001 // << default 5001 como você pediu
    );
    const password = process.env.REDIS_PASSWORD || undefined;
    const db = process.env.REDIS_DB !== undefined ? Number(process.env.REDIS_DB) : undefined;
...
const k = (id) => `flow:lock:welcome:${id}`;
/**
 * Tenta adquirir um lock por ticket por alguns segundos (TTL).
 * Se conseguir, retorna true; se já houver lock ativo, retorna false.
 */
async function acquireTicketWelcomeLock(ticketId, ttlSec = Number(process.env.FLOW_MENU_COOLDOWN_SEC || 8)) {
    try {
...
exports.acquireTicketWelcomeLock = acquireTicketWelcomeLock;
