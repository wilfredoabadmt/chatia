...
exports.startQueueProcess = exports.randomValue = exports.parseToMilliseconds = exports.messageQueue = exports.contactReconcilerQueue = exports.queueMonitor = exports.campaignQueue = exports.sendScheduledMessages = exports.scheduleMonitor = exports.userMonitor = void 0;
...
exports.userMonitor = new bull_1.default("UserMonitor", { redis: redisConfig });
exports.scheduleMonitor = new bull_1.default("ScheduleMonitor", { redis: redisConfig });
exports.sendScheduledMessages = new bull_1.default("SendSacheduledMessages", { redis: redisConfig });
exports.campaignQueue = new bull_1.default("CampaignQueue", { redis: redisConfig });
exports.queueMonitor = new bull_1.default("QueueMonitor", { redis: redisConfig });
exports.contactReconcilerQueue = new bull_1.default("ContactReconciler", { redis: redisConfig });
    ...
    try {
        const { count, rows: schedules } = await Schedule_1.default.findAndCountAll({
            where: {
                ...
                sentAt: null,
                sendAt: {
                    [sequelize_1.Op.gte]: (0, moment_1.default)().utc().format("YYYY-MM-DD HH:mm:ss"),
                    [sequelize_1.Op.lte]: (0, moment_1.default)().utc().add("30", "seconds").format("YYYY-MM-DD HH:mm:ss")
                }
            },
...
async function handleSendScheduledMessage(job) {
    ...
    try {
        ...
        if (schedule.valorIntervalo > 0 && ((0, lodash_1.isNil)(schedule.contadorEnvio) || schedule.contadorEnvio < schedule.enviarQuantasVezes)) {
            ...
            const timezone = await TimezoneService_1.default.getEffectiveTimezone(schedule.companyId);
            // Converter sendAt do UTC para timezone da empresa
            let novaData = moment_1.default.utc(schedule.sendAt).tz(timezone);
                ...
                if (campaign.confirmation && campaignShipping.confirmation === null) {
                    ...
                    if (campaign.mediaPath) {
                        ...
                        if (Object.keys(options).length) {
                            ...
                            if ("mimetype" in options && typeof options.mimetype === "string" && options.mimetype.startsWith("audio/")) {
...
async function handleProcessLanes() {
    const job = new CronJob('*/1 * * * *', async () => {
        logger_1.default.info("[KANBAN] Iniciando verificação de movimentação de lanes");
        const companies = await Company_1.default.findAll({
            include: [
                {
        ...
        logger_1.default.info(`[KANBAN] Empresas com kanban ativo: ${companies.length}`);
        companies.map(async (c) => {
            try {
                const companyId = c.id;
                logger_1.default.info(`[KANBAN] Processando empresa ${companyId}`);
                ...
                logger_1.default.info(`[KANBAN] Tickets encontrados para empresa ${companyId}: ${ticketTags.length}`);
                if (ticketTags.length > 0) {
                    // ✅ CORREÇÃO: Usar Promise.all ao invés de map para evitar race conditions
                    await Promise.all(ticketTags.map(async (t) => {
                        try {
                            logger_1.default.info(`[KANBAN] Ticket ${t.ticketId} - TagID: ${t.tagId} - TimeLane: ${t.tag.timeLane}h - NextLaneId: ${t.tag.nextLaneId}`);
                            if (!(0, lodash_1.isNil)(t?.tag.nextLaneId) && t?.tag.nextLaneId > 0 && t?.tag.timeLane > 0) {
                                ...
                                const nextTag = await Tag_1.default.findOne({
                                    where: {
                                        id: t?.tag.nextLaneId,
                                ...
                                if (!nextTag) {
                                    logger_1.default.error(`[KANBAN] nextTag ${t?.tag.nextLaneId} not found or doesn't belong to company ${companyId}`);
                                    return;
                                }
                                ...
                                logger_1.default.info(`[KANBAN] Ticket ${t.ticketId} - UpdatedAt: ${dataUltimaInteracaoChamado.toISOString()} - Limite: ${dataLimite.toISOString()} - Deve mover: ${dataUltimaInteracaoChamado < dataLimite}`);
                                if (dataUltimaInteracaoChamado < dataLimite) {
                                    ...
                                    const io = (0, socket_1.getIO)();
                                    const ticketUpdated = await (0, ShowTicketService_1.default)(t.ticketId, companyId);
                                    ...
                                    logger_1.default.info(`[KANBAN] ✅ Ticket ${t.ticketId} movido da tag ${t.tagId} para tag ${nextTag.id}`);
                                }
                                else {
                                    logger_1.default.info(`[KANBAN] ⏰ Ticket ${t.ticketId} ainda não atingiu o tempo limite`);
                                }
                            }
                            else {
                                logger_1.default.info(`[KANBAN] ⚠️ Ticket ${t.ticketId} não tem configuração válida (nextLaneId: ${t?.tag.nextLaneId}, timeLane: ${t?.tag.timeLane})`);
                            }
                        }
                        catch (error) {
                            Sentry.captureException(error);
                            logger_1.default.error(`[KANBAN] ❌ Erro ao mover ticket ${t.ticketId} da tag ${t.tagId}:`, error.message);
                        }
                    }));
                }
                else {
                    logger_1.default.info(`[KANBAN] Nenhum ticket com as condições necessárias para empresa ${companyId}`);
                }
            }
            catch (e) {
                Sentry.captureException(e);
                logger_1.default.error("[KANBAN] Erro no processamento de lanes:", e.message);
                throw e;
            }
        });
        logger_1.default.info("[KANBAN] Finalizado processamento de movimentação de lanes");
    });
    job.start();
    logger_1.default.info("[KANBAN] Cron job de processamento de lanes iniciado (executa a cada 1 minuto)");
}
...
async function handleWhatsapp() {
    ...
    const defaultTimezone = await Promise.resolve().then(() => __importStar(require("./services/TimezoneServices/TimezoneService"))).then(module => module.default.getDefaultTimezone().catch(() => 'America/Sao_Paulo'));
...
async function startQueueProcess() {
    ...
    exports.contactReconcilerQueue.process("Reconcile", async () => {
        try {
            const { runReconciler } = await Promise.resolve().then(() => __importStar(require("./services/ContactServices/ContactReconcilerService")));
            await runReconciler();
        }
        catch (e) {
            logger_1.default.error(`[Reconciler] Erro no job: ${e.message}`);
        }
    });
    exports.contactReconcilerQueue.add("Reconcile", {}, {
