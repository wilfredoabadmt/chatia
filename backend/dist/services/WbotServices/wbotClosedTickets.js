...
const handleOpenTickets = async (companyId, whatsapp) => {
    let currentTime;
    try {
        // Get current time in company timezone instead of hardcoded Brazil timezone
        currentTime = await DateHelper_1.default.getCurrentTime(companyId);
    }
    catch (error) {
        console.error("Error getting company timezone in handleOpenTickets:", error);
...
const handleNPSTickets = async (companyId, whatsapp) => {
    ...
    let dataLimite;
    try {
        // Use company timezone for calculating the expiration limit
        dataLimite = await DateHelper_1.default.subtractTime(companyId, new Date(), expiresTime, 'minutes');
    }
    catch (error) {
        console.error("Error calculating NPS expiration time with timezone:", error);
        // Fallback to moment without timezone
        dataLimite = (0, moment_1.default)().subtract(expiresTime, 'minutes').toDate();
    }
    const ticketsToClose = await Ticket_1.default.findAll({
        where: {
            status: "nps",
            companyId,
            whatsappId: whatsapp.id,
            updatedAt: { [sequelize_1.Op.lt]: dataLimite },
            imported: null
        }
    });
    if (ticketsToClose && ticketsToClose.length > 0) {
        ...
        await Promise.all(ticketsToClose.map(async (ticket) => {
            ...
            const currentTime = await DateHelper_1.default.getCurrentTime(companyId).catch(() => new Date());
            await ticketTraking.update({
