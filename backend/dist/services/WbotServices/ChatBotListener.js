...
const sendMessage = async (wbot, contact, ticket, body) => {
    const jid = await (0, IdentityResolverService_1.resolveOutgoingJid)(contact, ticket.isGroup);
    ...
    if (showChatBots.options) {
        ...
        const botButton = async () => {
            ...
            if (buttons.length > 0) {
                ...
                const dialogJid = await (0, IdentityResolverService_1.resolveOutgoingJid)(contact, ticket.isGroup);
                const send = await wbot.sendMessage(dialogJid, buttonMessage);
        ...
        const botList = async () => {
            ...
            if (sectionsRows.length > 0) {
                ...
                const listJid = await (0, IdentityResolverService_1.resolveOutgoingJid)(contact, ticket.isGroup);
                const sendMsg = await wbot.sendMessage(listJid, listMessage);
...
const backToMainMenu = async (wbot, contact, ticket, ticketTraking) => {
    ...
    const botText = async () => {
        ...
        const menuJid = await (0, IdentityResolverService_1.resolveOutgoingJid)(contact, ticket.isGroup);
        if (greetingMediaAttachment !== null) {
            ...
            const sentMessage = await wbot.sendMessage(menuJid, { ...optionsMsg });
            await (0, wbotMessageListener_1.verifyMediaMessage)(sentMessage, ticket, contact, ticketTraking, false, false, wbot);
        }
        else {
            const sentMessage = await wbot.sendMessage(menuJid, {
                text: body
            });
            await (0, wbotMessageListener_1.verifyMessage)(sentMessage, ticket, contact);
        }
