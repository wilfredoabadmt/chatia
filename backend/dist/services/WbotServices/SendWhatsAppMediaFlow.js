...
const processAudio = async (audio) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        (0, child_process_1.exec)(`${ffmpegBinary} -i ${audio} -vn -ab 128k -ar 44100 -f ipod ${outputAudio} -y`, (error, _stdout, _stderr) => {
            if (error)
                reject(error);
            //fs.unlinkSync(audio);
            resolve(outputAudio);
...
const processAudioFile = async (audio) => {
    ...
    return new Promise((resolve, reject) => {
        const ffmpegBinary = (0, ffmpeg_1.default)();
        (0, child_process_1.exec)(`${ffmpegBinary} -i ${audio} -vn -ar 44100 -ac 2 -b:a 192k ${outputAudio}`, (error, _stdout, _stderr) => {
            if (error)
                reject(error);
            //fs.unlinkSync(audio);
            resolve(outputAudio);
...
const typeSimulation = async (ticket, presence) => {
    ...
    const presenceJid = await (0, IdentityResolverService_1.resolveOutgoingJid)(contact, ticket.isGroup);
...
const SendWhatsAppMediaFlow = async ({ media, ticket, body, isFlow = false, isRecord = false }) => {
    try {
        ...
        let number = await (0, IdentityResolverService_1.resolveOutgoingJid)(contact, ticket.isGroup);
