...
const ListTicketsService = async ({ searchParam = "", pageNumber = "1", queueIds, tags, users, status, date, dateStart, dateEnd, updatedAt, showAll, userId, withUnreadMessages = "false", whatsappIds, statusFilters, companyId, sortTickets = "DESC", searchOnMessages = "false", unreplied = "false" }) => {
    ...
    whereCondition = {
        ...
        queueId: showTicketWithoutQueue ? { [sequelize_1.Op.or]: [{ [sequelize_1.Op.in]: queueIds }, null] } : { [sequelize_1.Op.in]: queueIds },
        companyId
    };
    ...
    if (status === "open") {
        ...
        whereCondition = {
            companyId,
            queueId: { [sequelize_1.Op.or]: [{ [sequelize_1.Op.in]: queueIds }, null] },
    ...
    if (showAll === "true" && (user.profile === "admin" || user.allUserChat === "enabled") && status !== "search") {
        if (user.allHistoric === "enabled" && showTicketWithoutQueue) {
            ...
            whereCondition = { companyId, queueId: { [sequelize_1.Op.or]: [{ [sequelize_1.Op.in]: queueIds }, null] } };
        }
        else if (user.allHistoric === "disabled" && !showTicketWithoutQueue) {
            whereCondition = { companyId, queueId: { [sequelize_1.Op.in]: queueIds } };
        }
    }
    ...
    if (status === "closed") {
        ...
        if (!showTicketAllQueues) {
            ...
            if (showAll === "false" && user.profile === "admin") {
                ...
                whereCondition2 = {
                    ...whereCondition2,
                    queueId: showAll === "true" || showTicketWithoutQueue ? { [sequelize_1.Op.or]: [{ [sequelize_1.Op.in]: queueIds }, null] } : { [sequelize_1.Op.in]: queueIds },
                };
            }
        ...
        whereCondition = {
            [sequelize_1.Op.or]: [
                ...
                },
                {
                    ...
                    queueId: showTicketWithoutQueue ? { [sequelize_1.Op.or]: [{ [sequelize_1.Op.in]: userQueueIds }, null] } : { [sequelize_1.Op.in]: userQueueIds },
                    unreadMessages: { [sequelize_1.Op.gt]: 0 },
                    companyId,
                    isGroup: showGroups ? { [sequelize_1.Op.or]: [true, false] } : false
                }
        ...
        if (status === "group" && (user.allowGroup || showAll === "true")) {
            whereCondition = {
                ...whereCondition,
                queueId: { [sequelize_1.Op.or]: [{ [sequelize_1.Op.in]: userQueueIds }, null] },
            };
        }
    }
    whereCondition = {
