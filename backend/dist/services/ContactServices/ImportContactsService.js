...
async function ImportContactsService(companyId, file) {
    ...
    const contacts = rows.map(row => {
        ...
        if ((0, lodash_2.has)(row, "followUp") ||
            (0, lodash_2.has)(row, "follow up") ||
            (0, lodash_2.has)(row, "Follow Up") ||
            (0, lodash_2.has)(row, "FollowUp") ||
            (0, lodash_2.has)(row, "observação") ||
            (0, lodash_2.has)(row, "Observação") ||
            (0, lodash_2.has)(row, "observacao") ||
            (0, lodash_2.has)(row, "Observacao")) {
            followUp = row["followUp"] || row["follow up"] || row["Follow Up"] || row["FollowUp"] ||
                row["observação"] || row["Observação"] || row["observacao"] || row["Observacao"] || "";
        }
        return { name, number, email, followUp, companyId };
    });
    const contactList = [];
