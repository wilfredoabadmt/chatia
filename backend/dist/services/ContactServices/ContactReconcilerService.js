...
exports.onLidMappingDiscovered = exports.runReconciler = void 0;
 ...
 */
async function mergeContacts(sourceId, targetId, reason) {
    const source = await Contact_1.default.findByPk(sourceId);
    const target = await Contact_1.default.findByPk(targetId);
    if (!source || !target)
        return;
    ...
    // Mover tickets
    const [ticketCount] = await Ticket_1.default.update({ contactId: target.id }, { where: { contactId: source.id } });
    // Mover mensagens
    const [messageCount] = await Message_1.default.update({ contactId: target.id }, { where: { contactId: source.id } });
    // Transferir identidades do source para o target
    try {
        await ContactIdentity_1.default.update({ contactId: target.id }, { where: { contactId: source.id } });
    }
    catch (e) {
        // Constraint violation = target já tem essa identidade. Deletar a duplicata.
        await ContactIdentity_1.default.destroy({ where: { contactId: source.id } });
    }
    // Copiar lid/jid do source para target se target não tem
    const updateData = {};
    if (!target.lid && source.lid)
        updateData.lid = source.lid;
    if (!target.jid && source.jid)
        updateData.jid = source.jid;
    ...
    logger_1.default.info(`[Reconciler] Merge: contato ${source.id} (${source.name}/${source.number}) ` +
 ...
 */
async function resolvePendingIdentities() {
    let resolved = 0;
    const pendings = await PendingIdentityResolution_1.default.findAll({
        where: { status: "pending" },
        limit: 50
    });
    for (const pending of pendings) {
        try {
            const jid = await (0, wbot_1.resolveLidToJidAsync)(pending.lidValue);
            if (!jid)
                continue;
            ...
            if (!contact) {
                ...
                const identity = await ContactIdentity_1.default.findOne({
                    where: {
                        companyId: pending.companyId,
                        identityValue: { [sequelize_1.Op.in]: [jid, realNumber, pending.lidValue] }
                    }
                });
                if (identity) {
                    contact = await Contact_1.default.findByPk(identity.contactId);
                }
            }
            if (contact) {
                // Registrar identidades
                await (0, IdentityResolverService_1.registerIdentities)(contact.id, pending.companyId, [
                ...
                resolved++;
                logger_1.default.info(`[Reconciler] Pendência ${pending.id} resolvida: LID ${pending.lidValue} → ` +
                    `contato ${contact.id} (${contact.name})`);
            }
        }
        catch (e) {
            logger_1.default.warn(`[Reconciler] Erro ao resolver pendência ${pending.id}: ${e.message}`);
        }
    }
    return resolved;
}
...
async function findAndMergeDuplicates() {
    ...
    try {
        ...
        const duplicates = await ContactIdentity_1.default.sequelize.query(`
    ...
    `, { type: "SELECT" });
        for (const dup of duplicates) {
            await mergeContacts(dup.source_id, dup.target_id, `identidade compartilhada: ${dup.identityType}=${dup.identityValue}`);
            merged++;
        }
        // Também verificar: contatos com LID number (>13 dígitos) + contato real
        // que compartilham um mapeamento LID conhecido
        const ghostContacts = await Contact_1.default.findAll({
            where: {
        ...
        for (const ghost of ghostContacts) {
            ...
            if (!lid)
                continue;
            const resolvedJid = await (0, wbot_1.resolveLidToJidAsync)(lid);
            if (!resolvedJid)
                continue;
            const realNumber = resolvedJid.replace(/@.*/, "");
            const realContact = await Contact_1.default.findOne({
                where: {
                    companyId: ghost.companyId,
                    number: realNumber,
                    id: { [sequelize_1.Op.ne]: ghost.id }
                }
            });
            if (realContact) {
                ...
                logger_1.default.info(`[Reconciler] Ghost ${ghost.id} atualizado: ${ghost.number} → ${realNumber}`);
            }
        }
    }
    catch (e) {
        logger_1.default.warn(`[Reconciler] Erro ao buscar duplicatas: ${e.message}`);
    }
    return merged;
}
/**
 ...
 */
async function expireOldPendings() {
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
    const [count] = await PendingIdentityResolution_1.default.update({ status: "expired" }, {
        where: {
            status: "pending",
            createdAt: { [sequelize_1.Op.lt]: sevenDaysAgo }
        }
    });
    if (count > 0) {
        logger_1.default.info(`[Reconciler] ${count} pendências expiradas`);
    }
    return count;
}
/**
    ...
    try {
        ...
        for (const pending of pendings) {
            ...
            if (contact) {
                ...
                logger_1.default.info(`[Reconciler] Pendência ${pending.id} resolvida imediatamente: ` +
    ...
    try {
        const ghost = await Contact_1.default.findOne({
            where: {
                ...
                [sequelize_1.Op.and]: [
                    { [sequelize_1.Op.or]: [{ isMerged: false }, { isMerged: null }] }
                ]
            }
        });
        if (!ghost)
            return;
        if (ghost.number === jidNumber)
            return; // já tem número correto
        const real = await Contact_1.default.findOne({
            where: {
                companyId: ghost.companyId,
                number: jidNumber,
                id: { [sequelize_1.Op.ne]: ghost.id }
        ...
        if (real) {
            ...
            // Registrar identidades
            await (0, IdentityResolverService_1.registerIdentities)(ghost.id, ghost.companyId, [
                { type: "phone", value: jidNumber, isPrimary: true },
                { type: "lid", value: lid },
                { type: "jid", value: jid }
            ]);
            logger_1.default.info(`[Reconciler] Ghost ${ghost.id} atualizado: ${lidNumber} → ${jidNumber}`);
            const io = (0, socket_1.getIO)();
        ...
        logger_1.default.warn(`[Reconciler] Erro ao fundir ghost: ${e.message}`);
    }
}
exports.onLidMappingDiscovered = onLidMappingDiscovered;
exports.default = { runReconciler, onLidMappingDiscovered };
