...
const UpdateContactService = async ({ contactData, contactId, companyId }) => {
    const { email, name, number, followUp, extraInfo, acceptAudioMessage, active, disableBot, remoteJid, wallets } = contactData;
    const contact = await Contact_1.default.findOne({
        where: { id: contactId },
        attributes: ["id", "name", "number", "channel", "email", "followUp", "companyId", "acceptAudioMessage", "active", "profilePicUrl", "remoteJid", "urlPicture"],
