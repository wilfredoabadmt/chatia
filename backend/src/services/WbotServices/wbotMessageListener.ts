...
import { msgDB, resolveLidToJid, setLidMapping } from "../../libs/wbot";
import {
  resolveMessageIdentity,
  resolveParticipantIdentity,
  registerIdentities,
  queuePendingResolution,
  resolveRemoteJidForMessage,
  resolveOutgoingJid
...
import { cancelFlowTimeout } from "../../helpers/FlowTimeoutHelper";
  ...
  if (
    ...
    if (participantAlt && !participantAlt.endsWith("@lid")) {
      ...
      if (senderPn) {
        ...
        const resolved = resolveLidToJid(jidNormalizedUser(senderId));
...
const getContactMessage = async (msg: proto.IWebMessageInfo, wbot: Session, companyId?: number) => {
  const isGroup = msg.key.remoteJid?.includes("g.us");

  if (isGroup) {
    // Para grupos, resolver identidade do participante (remetente)
    const participantId = await resolveParticipantIdentity(msg, wbot, companyId || 0);
    return {
  ...
  const identity = await resolveMessageIdentity(msg, wbot, companyId || 0);
...
const verifyContact = async (
  ...
  const identity = (msgContact as any)?._identity;
  ...
  if (identity?.isLidUnresolved && !contactData.isGroup) {
    if (identity.existingContactId) {
      const existing = await Contact.findByPk(identity.existingContactId);
      if (existing) return existing;
    }
    logger.warn(`[verifyContact] LID não resolvido para ${contactData.number}, tentando criar/buscar`);
  }
  ...
  if (contact && !contactData.isGroup) {
    const identities: Array<{ type: "lid" | "jid" | "phone"; value: string; isPrimary?: boolean }> = [];
    if (contact.number && contact.number.length <= 13) {
      identities.push({ type: "phone", value: contact.number, isPrimary: true });
    }
    if (identity?.lid) identities.push({ type: "lid", value: identity.lid });
    if (identity?.jid) identities.push({ type: "jid", value: identity.jid });
    if (contact.jid && contact.jid !== identity?.jid) {
      identities.push({ type: "jid", value: contact.jid });
    }
    if (identities.length > 0) {
      registerIdentities(contact.id, companyId, identities).catch(() => {});
    }
  }
...
        ack: msg.status,
        companyId: companyId,
        remoteJid: resolveRemoteJidForMessage(msg.key.remoteJid, msg, ticket.isGroup),
...
  const companyId = ticket.companyId;
  ...
  if (!msg.key.fromMe && body && body.length >= 3 && !isPrivate) {
    try {
      const { isAutoTranslateEnabled, detectAndTranslate } = await import("../TranslateService/TranslateService");
...
  const companyId = ticket.companyId;
  const contactJid = await resolveOutgoingJid(contact, ticket.isGroup);
...
if (!fromMe && ticket && chatbotEnabled /* && isFirstContact */) {
  ...
  if (ticket.flowWebhook) {
    ...
    await cancelFlowTimeout(ticket.id);
...
const handleMessage = async (
  ...
  try {
    ...
    if (!contact) {
      logger.warn(`[handleMessage] verifyContact retornou null para ${msgContact?.id}, ignorando mensagem`);
      return;
    }

    // JID resolvido para envio — usado em todos os sendMessage do fluxo
    const contactJid = await resolveOutgoingJid(contact, msg.key.remoteJid?.endsWith("@g.us") || false);
...
const filterMessages = (msg: WAMessage): boolean => {
  ...
  if (msg.key.remoteJid?.endsWith("@lid")) {
    ...
    if (senderPn) {
      ...
      const lidNorm = jidNormalizedUser(msg.key.remoteJid);
      const jidNorm = senderPn.includes("@") ? jidNormalizedUser(senderPn) : `${senderPn}@s.whatsapp.net`;
      setLidMapping(lidNorm, jidNorm);
      logger.info(`[filterMessages] LID resolvido via senderPn: ${lidNorm} → ${jidNorm}`);
    }
