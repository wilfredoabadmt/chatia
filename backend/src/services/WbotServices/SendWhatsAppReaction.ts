...
const SendWhatsAppReaction = async ({
  ...
  const contactForJid = ticket.contact || await (await import("../../models/Contact")).default.findByPk(ticket.contactId);
  const number = await resolveOutgoingJid(contactForJid, ticket.isGroup);
