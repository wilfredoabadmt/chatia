...
const sendMessage = async (
  ...
  const jid = await resolveOutgoingJid(contact, ticket.isGroup);
  ...
  if (showChatBots.options) {
    ...
    const botButton = async () => {
      ...
      if (buttons.length > 0) {
        ...
        const dialogJid = await resolveOutgoingJid(contact, ticket.isGroup);
        const send = await wbot.sendMessage(
          dialogJid,
          buttonMessage
        );
    ...
    const botList = async () => {
      ...
      if (sectionsRows.length > 0) {
        ...
        const listJid = await resolveOutgoingJid(contact, ticket.isGroup);
        const sendMsg = await wbot.sendMessage(
          listJid,
          listMessage
        );
...
const backToMainMenu = async (
  ...
  const botText = async () => {
    ...
    const menuJid = await resolveOutgoingJid(contact, ticket.isGroup);
