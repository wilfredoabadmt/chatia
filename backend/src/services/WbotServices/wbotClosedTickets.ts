...
const handleOpenTickets = async (companyId: number, whatsapp: Whatsapp) => {
  ...
  try {
    ...
    currentTime = await DateHelper.getCurrentTime(companyId);
  } catch (error) {
...
const handleNPSTickets = async (companyId: number, whatsapp: any) => {
  ...
  try {
    ...
    dataLimite = await DateHelper.subtractTime(companyId, new Date(), expiresTime, 'minutes');
  } catch (error) {
    console.error("Error calculating NPS expiration time with timezone:", error);
    // Fallback to moment without timezone
    dataLimite = moment().subtract(expiresTime, 'minutes').toDate();
  }
  ...
  if (ticketsToClose && ticketsToClose.length > 0) {
    ...
    await Promise.all(ticketsToClose.map(async ticket => {
      ...
      const currentTime = await DateHelper.getCurrentTime(companyId).catch(() => new Date());
