      ...
      try {
        if (contactId) {
          ...
          if (!cancelled && data.language) {
            setContactLanguage(data.language);
          }
        }
              ...
              {!privateMessageInputVisible && (
                <div className={classes.flexItem}>
                  <div className={classes.messageInputWrapper}>
                    {autoTranslateEnabled && contactLanguage && (
                      <Tooltip title={
                        (i18n.t("messagesInput.translateTooltip") || "Sua mensagem será traduzida para") +
                        " " + (LANGUAGE_NAMES_SHORT[contactLanguage] || contactLanguage.toUpperCase())
                      }>
                        <span className={classes.translateBadge}>
                          <span className={classes.translateFlag}>
                            {LANGUAGE_FLAGS[contactLanguage] || LANGUAGE_FLAGS[contactLanguage.split("-")[0]] || "🌐"}
                          </span>
                          {LANGUAGE_NAMES_SHORT[contactLanguage] || LANGUAGE_NAMES_SHORT[contactLanguage.split("-")[0]] || contactLanguage.toUpperCase()}
                        </span>
                      </Tooltip>
                    )}
                    <InputBase
