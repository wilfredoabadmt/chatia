...
const ScheduleModal = ({ open, onClose, scheduleId, contactId, cleanContact, reload }) => {
	...
	// Hook para obter timezone da empresa
	const { getCompanyTimezone, getCurrentTimeInTimezone } = useTimezone();
	const [companyTimezone, setCompanyTimezone] = useState("America/Sao_Paulo");
	const [currentTimeInTz, setCurrentTimeInTz] = useState("");
	...
	// Carregar timezone da empresa
	useEffect(() => {
		const loadTimezone = async () => {
			try {
				const tzData = await getCompanyTimezone(companyId);
				setCompanyTimezone(tzData.timezone);

				const currentTime = await getCurrentTimeInTimezone(companyId);
				setCurrentTimeInTz(currentTime);
			} catch (error) {
	...
	}, [open, companyId, getCompanyTimezone, getCurrentTimeInTimezone]);
	...
	useEffect(() => {
		...
		if (open) {
			try {
				...
				// Converter sendAt de UTC para timezone da empresa
				const tzData = await getCompanyTimezone(companyId);
				const sendAtInTz = moment.utc(data.sendAt).tz(tzData.timezone).format('YYYY-MM-DDTHH:mm');

				setSchedule(prevState => {
					return { ...prevState, ...data, sendAt: sendAtInTz };
					});
					console.log(data)
					if (data.whatsapp) {
									...
									<Grid item xs={12} md={6} xl={6}>
										<Field
											...
											helperText={
												...
												? errors.sendAt
												: `Timezone: ${companyTimezone}${currentTimeInTz ? ` | Hora atual: ${currentTimeInTz}` : ''}`
											}
											variant="outlined"
											fullWidth
											size="small"
