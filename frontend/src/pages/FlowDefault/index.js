 ...
 const { user, socket } = useContext(AuthContext);
  ...
  const handleDeleteWebhook = async webhookId => {
    try {
      ...
      toast.success(i18n.t("flowbuilder.toasts.deleteSuccess"));
    } catch (err) {
      toastError(err);
    }
  };
  ...
  const handleSaveDefault = async () => {
    ...
    if (configExist) {
      try {
        ...
        toast.success(i18n.t("connections.flowBuilder.updatedSuccess"));
      } catch (err) {
        toastError(err);
      }
    } else {
  ...
  return (
    <MainContainer className={classes.mainContainer}>
      ...
      >
        {deletingContact
          ? i18n.t("connections.flowBuilder.deleteConfirmation")
          : `${i18n.t("contacts.confirmationModal.importMessage")}`}
      </ConfirmationModal>
      <MainHeader>
        <Title>{i18n.t("connections.flowBuilder.title")}</Title>
      </MainHeader>
      <Paper
      ...
      >
        <Stack sx={{ padding: '12px', position: 'relative' }}>
          <Stack sx={{ position: 'absolute', right: 0 }}>
            ...
            }}>{i18n.t("connections.flowBuilder.save")}</Button>
          </Stack>
          <Stack gap={'12px'}>
            <Typography fontSize={18} fontWeight={700}>{i18n.t("connections.flowBuilder.welcomeFlow")}</Typography>
            <Typography fontSize={12}>
              {i18n.t("connections.flowBuilder.welcomeFlowDescription")}
            </Typography>
            {!loading && (<Autocomplete
              ...
              renderInput={params => (
                <TextField
                  {...params}
                  variant="outlined"
                  placeholder={i18n.t("campaigns.modal.flowPlaceholder")}
                />
              )}
          ...
          <Stack gap={'12px'} marginTop={4}>
            <Typography fontSize={18} fontWeight={700}>{i18n.t("connections.flowBuilder.defaultResponseFlow")}</Typography>
            <Typography fontSize={12}>
              {i18n.t("connections.flowBuilder.defaultResponseFlowDescription")}
            </Typography>
            {!loading && (<Autocomplete
              disablePortal
              id="combo-box-demo"
